 ...
 */
var mchntMngController = function ($scope, $common, dataDeliver, $state) {
	//初始化变量
    ...
    function queryMchntInfo() {
        ...
        var data = {
            ...
            mchntInfo: $scope.condition
        }
        //这两行代码解决: 传对象参数为空的问题;
        var dataString = JSON.stringify(data);
        data = JSON.parse(dataString);
        var promise = $common.get_asyncData("mchntMngService/findMchntInfosByPage", data);
        promise.then(function (res) {
            var data;
            if (res.errorCode == "SUC") {
                data = res.contextData;
                $scope.mchntData.ds = data.records;
    ...
    $scope.add_onClick = function () {
        $common.get_ngDialog('mchntMng/Add/mchntInfoAdd.html', $scope, ["$scope", function (dialogScope) {
        	//打开层的字段初始化值
            dialogScope.mchntInfoAdd = {
            	mchntId: "",
                mchntValidSt: "Y"
            };
            //提交的数据
            var data = {
            	mchntInfo: dialogScope.mchntInfoAdd
            }
            dialogScope.confirm_onClick = function () {
                if ($.doValidate("mchntInfoAddForm")) {
                    if (addMchntInfoCheck(dialogScope.mchntInfoAdd, $common) == true) {
                        var promise = $common.get_asyncData("mchntMngService/addMchntInf", data);
                        promise.then(function (res) {
                            if (res.errorCode == "SUC") {
                                $common.get_tipDialog('新增成功！', 'showSuccess');
                                dialogScope.closeThisDialog();
                                var merId = res.contextData.value;
                                var data = {
                                	mchntId: merId
                                }
                                //$state.go("userDetail");//跳转
    ...
    $scope.delete_onClick = function () {
        if ($scope.selected.select.length < 1) {
            $common.get_tipDialog('请选择一条商户记录!', 'showInform')
        } else {
            $common.get_tipDialog('确定删除商户吗？', 'showTip').then(function () {
                var data = {
                	mchntInfo: $scope.selected.select[0]
                }
                var promise1 = $common.get_asyncData("mchntMngService/deleteMchntInfo", data);
                promise1.then(function (res) {
                    if (res.errorCode == "SUC" && "" != res.contextData.value) {
                    	...
                    	queryMchntInfo();
                    	$common.get_tipDialog('删除商户成功！', 'showSuccess');
                    } else {
                        $common.get_tipDialog(res.errorMsg, 'showInform');
                    }
                })
    ...
    $scope.detail_onClick = function () {
        if ($scope.selected.select.length != 1) {
            $common.get_tipDialog('只能选择一条商户记录！', 'showInform');
        } else {
            var data = {
            	mchntId: $scope.selected.select[0].mchntId,
            	state : "readonly"
            };
            $state.go("mchntDetail");//跳转
